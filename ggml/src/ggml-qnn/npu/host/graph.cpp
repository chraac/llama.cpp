...
npu_graph::npu_graph(ggml_cgraph * cgraph, remote_handle64 device_handle) : _device_handle(device_handle) {
    ...
    for (int i = 0; i < cgraph->n_nodes; ++i) {
        ...
        auto * tensor_obj = npu_tensor::from_ggml_tensor(node);
        if (!tensor_obj) {
            continue;
        }
        ...
        for (size_t j = 0; j < GGML_MAX_SRC && node->src[i]; ++j) {
            auto * src = npu_tensor::from_ggml_tensor(node->src[i]);
